...
function renderSmlFinanceView() {
  ...
  document.querySelector("#smlFileInput")?.addEventListener("change", async (event) => { const file = event.target.files?.[0]; if (!file) return; try { const fileBase64 = await fileToDataUrl(file); state.smlFinanceDuplicateSelections = {}; state.smlFinanceExtraction = await api("/api/sml-finance/import-preview", { method: "POST", body: JSON.stringify({ fileBase64 }) }); renderSmlFinanceView(); } catch (error) { alert(error.message); } });
