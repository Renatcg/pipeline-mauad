...
const state = {
  ...
  marketingCashflowGroupExpanded: {},
  marketingLaunchTab: "planned",
  marketingFundModalOpen: false,
  marketingEditingFundId: "",
  marketing: { actualExpenses: [], reconciliationQueue: [], provisions: [], commitments: [], marketingFunds: [], budgetGroups: [], budgetCategories: [], budgetEntries: [] },
...
async function loadState() {
  ...
  state.marketing = data.marketing || { actualExpenses: [], reconciliationQueue: [], provisions: [], commitments: [], marketingFunds: [], budgetGroups: [], budgetCategories: [], budgetEntries: [] };
  ...
  return [
    {
      ...
      contactEmail: "mariana@casadeeventos.com.br",
      contactPhone: "(21) 99999-1122",
      ...
      activities: [
        { id: "act-1", name: "Definição do conceito", duration: 4, start: "2026-08-17", end: "2026-08-20", predecessorId: "", linkType: "T-I" },
        { id: "act-2", name: "Produção dos assets", duration: 8, start: "2026-08-21", end: "2026-08-28", predecessorId: "act-1", linkType: "T-I" },
        { id: "act-3", name: "Confirmação de fornecedores", duration: 6, start: "2026-08-24", end: "2026-08-29", predecessorId: "act-1", linkType: "T-I" },
        { id: "act-4", name: "Convites e confirmações", duration: 14, start: "2026-09-01", end: "2026-09-14", predecessorId: "act-2", linkType: "T-I" },
        { id: "act-5", name: "Montagem e realização", duration: 2, start: "2026-09-17", end: "2026-09-18", predecessorId: "act-3", linkType: "T-I" }
      ],
      items: [
        { id: "item-1", activityId: "act-3", type: "Serviço", description: "Locação do espaço e mobiliário", quantity: 1, unit: "pacote", quotes: [{ id: "item-1-q1", supplier: "Experiência Eventos", value: 14800, attachment: "proposta-experiencia.pdf" }, { id: "item-1-q2", supplier: "Casa Sete Produções", value: 13250, attachment: "cotacao-casa-sete.pdf" }] },
        { id: "item-2", activityId: "act-5", type: "Serviço", description: "Buffet para convidados", quantity: 120, unit: "pessoas", quotes: [{ id: "item-2-q1", supplier: "Buffet Primavera", value: 10800, attachment: "proposta-buffet.pdf" }] },
        { id: "item-3", activityId: "act-2", type: "Produto", description: "Kits de boas-vindas", quantity: 120, unit: "unidades", quotes: [] },
        { id: "item-4", activityId: "act-5", type: "Serviço", description: "Sonorização e iluminação", quantity: 1, unit: "diária", quotes: [] }
      ],
      quotes: [
    ...
    },
    {
      ...
      contactEmail: "rafael@acaoregional.com.br",
      contactPhone: "(24) 98888-4400",
      ...
      activities: [
        { id: "act-6", name: "Licenciamento do local", duration: 10, start: "2026-09-08", end: "2026-09-17", predecessorId: "", linkType: "T-I" },
        { id: "act-7", name: "Produção de materiais", duration: 12, start: "2026-09-18", end: "2026-09-29", predecessorId: "act-6", linkType: "T-I" },
...
function marketingCashflowContext(project = state.marketingBudgetProject || "TODOS") {
  const actualEntries = (state.marketing?.actualExpenses || []).filter((item) => project === "TODOS" || item.project === project);
  const plannedEntries = (state.marketing?.budgetEntries || []).filter((item) => project === "TODOS" || item.project === project);
  const committedEntries = marketingCommittedEntries(project);
  const todayMonth = new Date().toISOString().slice(0, 7);
  const allCompetences = [...actualEntries.map((item) => String(item.paymentDate).slice(0, 7)), ...plannedEntries.map((item) => item.competence), ...committedEntries.map((item) => item.competence), todayMonth].filter((item) => /^\d{4}-\d{2}$/.test(item)).sort();
  const firstMonth = allCompetences[0] || todayMonth;
  const lastMonth = allCompetences.at(-1) || todayMonth;
  const timelineMonths = marketingMonthRange(firstMonth, lastMonth);
  const appliedStart = state.marketingTimelineStart || firstMonth;
  const appliedEnd = state.marketingTimelineEnd || lastMonth;
  const visibleMonths = marketingMonthRange(appliedStart, appliedEnd);
  return { project, actualEntries, plannedEntries, committedEntries, firstMonth, lastMonth, timelineMonths, appliedStart, appliedEnd, visibleMonths };
}

function marketingEntryGroup(entry, movement) {
  if (movement === "actual") return entry.financialPlanName || "Sem grupo";
  return entry.group || "Sem grupo";
}

function marketingEntryCompetence(entry, movement) {
  return String(movement === "actual" ? entry.paymentDate : entry.competence).slice(0, 7);
}

function marketingEntryAmount(entry, movement) {
  return Number(movement === "actual" ? entry.paidAmount : entry.amount || 0);
}

function marketingEntryLabel(entry, movement) {
  if (movement === "planned") return entry.categoryName || entry.notes || "Lançamento previsto";
  if (movement === "committed") return entry.sourceName || entry.sourceType || "Compromisso";
  return entry.notes || `${entry.creditorName || "Credor"} · ${entry.document || "sem documento"}`;
}

function renderMarketingCashflowRows(key, label, entries, months, groups) {
  const movement = key;
  const totals = months.map((month) => entries.filter((item) => marketingEntryCompetence(item, movement) === month).reduce((sum, item) => sum + marketingEntryAmount(item, movement), 0));
  const expanded = Boolean(state.marketingCashflowExpanded[key]);
  const parent = `<tr class="marketing-tree-parent"><th><button type="button" class="marketing-tree-toggle" data-toggle-marketing-cashflow="${key}" aria-expanded="${expanded}">${expanded ? "−" : "+"}</button>${escapeHtml(label)}</th>${totals.map((value) => `<td>${value ? escapeHtml(brl(value)) : "-"}</td>`).join("")}<td><strong>${escapeHtml(brl(totals.reduce((sum, value) => sum + value, 0)))}</strong></td></tr>`;
  if (!expanded) return parent;
  const groupNames = [...groups, ...entries.map((item) => marketingEntryGroup(item, movement)).filter((group) => !groups.includes(group))];
  return parent + groupNames.map((group) => {
    const groupEntries = entries.filter((item) => marketingEntryGroup(item, movement) === group);
    const groupValues = months.map((month) => groupEntries.filter((item) => marketingEntryCompetence(item, movement) === month).reduce((sum, item) => sum + marketingEntryAmount(item, movement), 0));
    const groupKey = `${key}:${group}`;
    const groupExpanded = Boolean(state.marketingCashflowGroupExpanded[groupKey]);
    const groupRow = `<tr class="marketing-tree-child"><th>${groupEntries.length ? `<button type="button" class="marketing-tree-toggle" data-toggle-marketing-cashflow-group="${escapeHtml(groupKey)}" aria-expanded="${groupExpanded}">${groupExpanded ? "−" : "+"}</button>` : '<span class="marketing-tree-spacer"></span>'}${escapeHtml(group)}</th>${groupValues.map((value) => `<td>${value ? escapeHtml(brl(value)) : "-"}</td>`).join("")}<td>${escapeHtml(brl(groupValues.reduce((sum, value) => sum + value, 0)))}</td></tr>`;
    if (!groupExpanded) return groupRow;
    return groupRow + groupEntries.map((entry) => {
      const competence = marketingEntryCompetence(entry, movement);
      const amount = marketingEntryAmount(entry, movement);
      return `<tr class="marketing-tree-entry"><th title="${escapeHtml(marketingEntryLabel(entry, movement))}">${escapeHtml(marketingEntryLabel(entry, movement))}</th>${months.map((month) => `<td>${month === competence ? escapeHtml(brl(amount)) : "-"}</td>`).join("")}<td>${escapeHtml(brl(amount))}</td></tr>`;
    }).join("");
  }).join("");
}

function renderMarketingCashflowView() {
  const context = marketingCashflowContext();
  const { project, actualEntries, plannedEntries, committedEntries, timelineMonths, appliedStart, appliedEnd, visibleMonths } = context;
  const groups = state.marketing?.budgetGroups || [];
  const planned = marketingValuesByMonths(plannedEntries, visibleMonths, "competence", "amount");
  const committed = marketingValuesByMonths(committedEntries, visibleMonths, "competence", "amount");
  const actual = marketingValuesByMonths(actualEntries, visibleMonths, "paymentDate", "paidAmount");
  const balance = visibleMonths.map((_, index) => planned[index] - committed[index] - actual[index]);
  let running = 0;
  const accumulated = balance.map((value) => (running += value));
  const alerts = visibleMonths.map((month, index) => ({ month, planned: planned[index], committed: committed[index], rate: planned[index] ? (actual[index] + committed[index]) / planned[index] : (committed[index] ? 1 : 0) })).filter((item) => (item.planned && item.rate >= 0.6) || (!item.planned && item.committed));
  return `<section class="panel marketing-panel"><div class="panel-head marketing-cashflow-head"><div><h2>Fluxo de caixa de Marketing</h2><p class="muted-copy">Previsto por competência; realizado pela data da baixa.</p></div><div class="row-actions marketing-cashflow-filters"><label class="compact-select"><span>Empreendimento</span><select id="marketingBudgetProject"><option value="TODOS">Todos</option>${(state.projects || []).map((item) => `<option value="${escapeHtml(item)}" ${project === item ? "selected" : ""}>${escapeHtml(item)}</option>`).join("")}</select></label><span class="marketing-date-choice">Início <strong>${escapeHtml(marketingMonthLabel(state.marketingTimelineDraftStart || appliedStart))}</strong></span><span class="marketing-date-choice">Fim <strong>${escapeHtml(marketingMonthLabel(state.marketingTimelineDraftEnd || appliedEnd))}</strong></span><button type="button" class="secondary" data-apply-marketing-timeline>Filtrar</button></div></div><div class="marketing-month-timeline is-transparent"><div class="marketing-timeline-track" style="--timeline-count:${timelineMonths.length}">${timelineMonths.map((month, index) => { const selected = month >= (state.marketingTimelineDraftStart || appliedStart) && month <= (state.marketingTimelineDraftEnd || state.marketingTimelineDraftStart || appliedEnd); return `<button type="button" class="${selected ? "selected" : ""} size-${index % 5}" data-marketing-timeline-month="${month}" title="${escapeHtml(marketingMonthLabel(month))}"><i></i><span>${escapeHtml(marketingMonthLabel(month))}</span></button>`; }).join("")}</div></div>${alerts.length ? `<div class="marketing-budget-alerts">${alerts.map((item) => `<span class="${marketingAttainmentClass(item.rate)}">${escapeHtml(marketingMonthLabel(item.month))}: ${item.planned ? `${Math.round(item.rate * 100)}% atingido` : "sem previsto"}</span>`).join("")}</div>` : ""}<div class="table-wrap marketing-cashflow"><table><thead><tr><th>Movimento</th>${visibleMonths.map((month) => `<th>${escapeHtml(marketingMonthLabel(month))}</th>`).join("")}<th>Total</th></tr></thead><tbody>${renderMarketingCashflowRows("planned", "Previsto", plannedEntries, visibleMonths, groups)}${renderMarketingCashflowRows("committed", "Comprometido", committedEntries, visibleMonths, groups)}${renderMarketingCashflowRows("actual", "Realizado", actualEntries, visibleMonths, groups)}<tr class="marketing-balance-row"><th>Saldo do mês</th>${balance.map((value) => `<td>${escapeHtml(brl(value))}</td>`).join("")}<td>${escapeHtml(brl(balance.reduce((sum, value) => sum + value, 0)))}</td></tr><tr class="marketing-balance-row accumulated"><th>Saldo acumulado</th>${accumulated.map((value) => `<td>${escapeHtml(brl(value))}</td>`).join("")}<td>${escapeHtml(brl(accumulated.at(-1) || 0))}</td></tr></tbody></table></div></section>`;
}

function renderMarketingRealizedLaunches() {
  const project = state.marketingBudgetProject || "TODOS";
  const expenses = (state.marketing?.actualExpenses || []).filter((item) => project === "TODOS" || item.project === project).filter((item) => !state.marketingExpenseSearch || [item.creditorName, item.financialPlanName, item.document, item.notes].some((value) => String(value || "").toLocaleLowerCase("pt-BR").includes(state.marketingExpenseSearch.toLocaleLowerCase("pt-BR")))).sort((a, b) => String(b.paymentDate).localeCompare(String(a.paymentDate)));
  const pending = (state.marketing?.reconciliationQueue || []).filter((item) => item.status === "pending");
  return `<section class="panel marketing-panel"><div class="panel-head"><div><h2>Gastos realizados</h2><p class="muted-copy">${numberPt(expenses.length)} lançamento(s) · ${escapeHtml(brl(expenses.reduce((sum, item) => sum + Number(item.paidAmount || 0), 0)))}</p></div><div class="row-actions"><label class="compact-select"><span>Empreendimento</span><select id="marketingBudgetProject"><option value="TODOS">Todos</option>${(state.projects || []).map((item) => `<option value="${escapeHtml(item)}" ${project === item ? "selected" : ""}>${escapeHtml(item)}</option>`).join("")}</select></label><label class="compact-select"><span>Busca</span><input id="marketingExpenseSearch" value="${escapeHtml(state.marketingExpenseSearch)}" placeholder="Credor, documento ou descrição"></label></div></div><div class="table-wrap marketing-expenses-table"><table><thead><tr><th>Pagamento</th><th>Empreendimento</th><th>Plano financeiro</th><th>Credor</th><th>Documento</th><th>Descrição</th><th>Valor pago</th><th>Origem</th></tr></thead><tbody>${expenses.slice(0, 200).map((item) => `<tr><td>${escapeHtml(dateLabel(item.paymentDate))}</td><td>${escapeHtml(item.project)}</td><td>${escapeHtml(item.financialPlanName || "—")}</td><td>${escapeHtml(item.creditorName || "—")}</td><td>${escapeHtml(item.document || "—")}</td><td title="${escapeHtml(item.notes || "")}">${escapeHtml(item.notes || "—")}</td><td class="money">${escapeHtml(brl(item.paidAmount))}</td><td><span class="chip">${item.source === "provisioning" ? "Provisionamento" : item.source === "manual_reconciliation" ? "Conciliação manual" : "Histórico"}</span></td></tr>`).join("")}</tbody></table></div></section><section class="panel marketing-panel"><div class="panel-head"><div><h2>Fila de conciliação manual</h2><p class="muted-copy">Itens sem correspondência exigem criação manual.</p></div><span class="chip">${numberPt(pending.length)} pendente(s)</span></div><div class="table-wrap"><table><thead><tr><th>Pagamento</th><th>Empreendimento</th><th>Credor</th><th>Documento</th><th>Valor</th><th>Ação</th></tr></thead><tbody>${pending.map((item) => `<tr><td>${escapeHtml(dateLabel(item.expense?.paymentDate))}</td><td>${escapeHtml(item.expense?.project || "—")}</td><td>${escapeHtml(item.expense?.creditorName || "—")}</td><td>${escapeHtml(item.expense?.document || "—")}</td><td>${escapeHtml(brl(item.expense?.paidAmount || 0))}</td><td><button type="button" data-create-marketing-historical="${escapeHtml(item.id)}">Criar novo gasto histórico</button></td></tr>`).join("") || '<tr><td colspan="6">Nenhum item aguardando conciliação.</td></tr>'}</tbody></table></div></section>`;
}

function renderMarketingFundModal() {
  if (!state.marketingFundModalOpen) return "";
  const fund = (state.marketing?.marketingFunds || []).find((item) => item.id === state.marketingEditingFundId);
  return `<div class="modal-backdrop" data-close-marketing-fund><section class="modal-card"><div class="panel-head"><div><h2>${fund ? "Editar Verba de Marketing" : "Nova Verba de Marketing"}</h2></div><button class="icon" type="button" data-close-marketing-fund>×</button></div><form id="marketingFundForm" class="form-grid"><div class="field full"><label>Empreendimento</label><select name="project" required>${(state.projects || []).map((item) => `<option value="${escapeHtml(item)}" ${fund?.project === item ? "selected" : ""}>${escapeHtml(item)}</option>`).join("")}</select></div><div class="field"><label>VGV estimado</label><input name="estimatedVgv" type="number" min="0" step="0.01" value="${fund?.estimatedVgv || ""}" required></div><div class="field"><label>% geral de verba de marketing</label><input name="marketingPercent" type="number" min="0" step="0.01" value="${fund?.marketingPercent || ""}" required></div><div class="field full marketing-fund-preview"><span>Verba calculada</span><strong data-marketing-fund-preview>${escapeHtml(brl(fund?.totalBudget || 0))}</strong></div><div class="field full"><button class="primary" type="submit">${fund ? "Salvar alterações" : "Criar VM"}</button></div></form></section></div>`;
}

function renderMarketingFunds() {
  const expenses = state.marketing?.actualExpenses || [];
  const funds = state.marketing?.marketingFunds || [];
  return `<section class="panel marketing-panel"><div class="panel-head"><div><h2>Verbas de Marketing</h2><p class="muted-copy">Limite geral calculado sobre o VGV estimado de cada empreendimento.</p></div><button class="primary" type="button" data-new-marketing-fund>Nova VM</button></div><div class="table-wrap marketing-funds-table"><table><thead><tr><th>Empreendimento</th><th>VGV estimado</th><th>% MKT</th><th>Verba total</th><th>Total gasto</th><th>Disponível</th><th>% consumido</th><th>Ações</th></tr></thead><tbody>${funds.map((fund) => { const spent = expenses.filter((item) => item.project === fund.project).reduce((sum, item) => sum + Number(item.paidAmount || 0), 0); const remaining = Number(fund.totalBudget || 0) - spent; const consumed = fund.totalBudget ? spent / fund.totalBudget : 0; return `<tr><td>${escapeHtml(fund.project)}</td><td>${escapeHtml(brl(fund.estimatedVgv))}</td><td>${numberPt(fund.marketingPercent)}%</td><td>${escapeHtml(brl(fund.totalBudget))}</td><td>${escapeHtml(brl(spent))}</td><td class="${remaining < 0 ? "negative" : ""}">${escapeHtml(brl(remaining))}</td><td><div class="marketing-consumption"><span><i style="width:${Math.min(100, consumed * 100)}%"></i></span><strong>${numberPt(consumed * 100)}%</strong></div></td><td>${renderSettingsActionMenu(`marketing-fund-${fund.id}`, [`<button type="button" data-edit-marketing-fund="${escapeHtml(fund.id)}">Editar e alterar</button>`])}</td></tr>`; }).join("") || '<tr><td colspan="8">Nenhuma Verba de Marketing cadastrada.</td></tr>'}</tbody></table></div></section>${renderMarketingFundModal()}`;
}

function renderMarketingLaunchesView() {
  const tabs = [["planned", "Previstos"], ["actual", "Realizados"], ["funds", "Verba mkt"]];
  return `<div class="tabs marketing-launch-tabs">${tabs.map(([id, label]) => `<button type="button" class="${state.marketingLaunchTab === id ? "active" : ""}" data-marketing-launch-tab="${id}">${label}</button>`).join("")}</div>${state.marketingLaunchTab === "planned" ? renderMarketingBudgetEntriesScreen(state.marketingBudgetProject || "TODOS").replace(/<button[^>]+data-close-marketing-budget-entries[^>]*>.*?<\/button>/, "") : state.marketingLaunchTab === "actual" ? renderMarketingRealizedLaunches() : renderMarketingFunds()}`;
}

function renderMarketingActionsMock() {
  const actions = marketingMockActions().sort((a, b) => String(b.eventDate).localeCompare(String(a.eventDate)));
  if (state.marketingEditingActionId) return renderMarketingEventEditorMock(actions);
  return `
    <section class="panel marketing-panel">
      <div class="panel-head"><div><h2>Ações e eventos</h2><p class="muted-copy">Eventos ordenados da data mais recente para a mais antiga.</p></div><button class="primary" type="button" data-marketing-mock-notice>Nova ação</button></div>
      <div class="table-wrap"><table class="marketing-events-table"><thead><tr><th>Data</th><th>Evento</th><th>Empreendimento</th><th>Local</th><th>Status</th><th>Ações</th></tr></thead><tbody>
        ${actions.map((action) => `<tr><td><strong>${escapeHtml(dateLabel(action.eventDate))}</strong></td><td>${escapeHtml(action.name)}</td><td>${escapeHtml(action.project)}</td><td>${escapeHtml(action.location)}</td><td><span class="status-ok">Em planejamento</span></td><td>${renderSettingsActionMenu(`marketing-event-${action.id}`, [`<button type="button" data-edit-marketing-event="${action.id}">Editar evento</button>`])}</td></tr>`).join("")}
      </tbody></table></div>
    </section>
  `;
}
...
function marketingActivityCost(action, activityId) {
  return (action.items || []).filter((item) => item.activityId === activityId).reduce((total, item) => {
    const winnerId = state.marketingItemWinnerSelections[`${action.id}:${item.id}`];
    const winner = marketingItemQuotes(action, item).find((quote) => quote.id === winnerId);
    return total + Number(winner?.value || 0);
  }, 0);
}
...
function renderMarketingActivityGantt(action, activities = []) {
  ...
  const width = days.length * 44;
  return `<div class="marketing-gantt-scroll"><div class="marketing-gantt" style="--timeline-width:${width}px"><div class="marketing-gantt-head"><span>Atividade</span><div class="marketing-gantt-days">${days.map((day) => `<span class="${[0, 6].includes(day.getDay()) ? "weekend" : ""}">${String(day.getDate()).padStart(2, "0")}/${String(day.getMonth() + 1).padStart(2, "0")}</span>`).join("")}</div></div>${activities.map((activity) => {
...
function renderMarketingEventEditorMock(actions) {
  const action = actions.find((item) => item.id === state.marketingEditingActionId) || actions[0];
  const activities = marketingActivitiesForAction(action);
  const activityTab = state.marketingEventDetailTab === "activities";
  return `
...
function renderMarketingView() {
  ...
  renderShell(`
    ...
    ${state.marketingTab === "budget" ? renderMarketingCashflowView() : state.marketingTab === "launches" ? renderMarketingLaunchesView() : state.marketingTab === "actions" ? renderMarketingActionsMock() : ""}
  ...
  document.querySelectorAll("[data-toggle-marketing-cashflow-group]").forEach((button) => button.addEventListener("click", () => {
    const key = button.dataset.toggleMarketingCashflowGroup;
    state.marketingCashflowGroupExpanded[key] = !state.marketingCashflowGroupExpanded[key];
    renderMarketingViewAtScroll();
  }));
  document.querySelectorAll("[data-marketing-launch-tab]").forEach((button) => button.addEventListener("click", () => { state.marketingLaunchTab = button.dataset.marketingLaunchTab; state.marketingFundModalOpen = false; renderMarketingView(); }));
  document.querySelector("[data-new-marketing-fund]")?.addEventListener("click", () => { state.marketingEditingFundId = ""; state.marketingFundModalOpen = true; renderMarketingView(); });
  document.querySelectorAll("[data-edit-marketing-fund]").forEach((button) => button.addEventListener("click", () => { state.marketingEditingFundId = button.dataset.editMarketingFund; state.marketingFundModalOpen = true; renderMarketingView(); }));
  document.querySelectorAll("[data-close-marketing-fund]").forEach((element) => element.addEventListener("click", (event) => { if (event.target !== element && element.classList.contains("modal-backdrop")) return; state.marketingFundModalOpen = false; state.marketingEditingFundId = ""; renderMarketingView(); }));
  const marketingFundForm = document.querySelector("#marketingFundForm");
  const updateMarketingFundPreview = () => {
    if (!marketingFundForm) return;
    const vgv = Number(marketingFundForm.elements.estimatedVgv?.value || 0);
    const percent = Number(marketingFundForm.elements.marketingPercent?.value || 0);
    const output = marketingFundForm.querySelector("[data-marketing-fund-preview]");
    if (output) output.textContent = brl(vgv * percent / 100);
  };
  marketingFundForm?.elements.estimatedVgv?.addEventListener("input", updateMarketingFundPreview);
  marketingFundForm?.elements.marketingPercent?.addEventListener("input", updateMarketingFundPreview);
  marketingFundForm?.addEventListener("submit", async (event) => {
    event.preventDefault();
    const form = new FormData(event.currentTarget);
    const button = event.currentTarget.querySelector("button[type=submit]");
    try {
      setButtonBusy(button, true, "Salvando...");
      const path = state.marketingEditingFundId ? `/api/marketing/funds/${encodeURIComponent(state.marketingEditingFundId)}` : "/api/marketing/funds";
      const result = await api(path, { method: state.marketingEditingFundId ? "PATCH" : "POST", body: JSON.stringify({ project: form.get("project"), estimatedVgv: Number(form.get("estimatedVgv")), marketingPercent: Number(form.get("marketingPercent")) }) });
      state.marketing = result.marketing;
      state.marketingFundModalOpen = false;
      state.marketingEditingFundId = "";
      renderMarketingView();
    } catch (error) {
      setButtonBusy(button, false);
      alert(error.message);
...
function renderSmlFinanceView() {
  ...
  const rows = activeRows.map((sale) => `<tr><td>${escapeHtml(sale.unit)}</td><td>${escapeHtml(sale.client)}</td><td>${sale.signedAt ? escapeHtml(dateLabel(sale.signedAt)) : "—"}</td><td>${money(sale.contractValue)}</td><td>${money(sale.signalValue || 0)}</td><td>${money(sale.financingValue || 0)}</td><td>${money(sale.commissionValue)}</td><td>${escapeHtml(sale.realEstate || "")}</td><td class="sml-zero-cell" ${sale.zeroEntry ? `data-sml-zero-entry="${sale.id}" title="Botão direito para desmarcar"` : ""}>${sale.zeroEntry ? '<span class="sml-zero-check">✓</span>' : "—"}</td><td><span class="chip">${escapeHtml(sale.status)}</span></td><td>—</td></tr>`).join("");
  const preview = state.smlFinanceExtraction?.preview || [];
  const fileConflicts = state.smlFinanceExtraction?.fileConflicts || [];
  const databaseConflicts = state.smlFinanceExtraction?.databaseConflicts || [];
  const isSelectedFileSale = (sale) => !sale.fileConflict || state.smlFinanceDuplicateSelections[sale.unit] === sale.id;
  const importablePreview = preview.filter((sale) => !sale.duplicate && isSelectedFileSale(sale));
  const unresolvedFileConflicts = fileConflicts.filter((conflict) => !state.smlFinanceDuplicateSelections[conflict.unit]);
  const conflictBoxes = `${databaseConflicts.length ? `<section class="sml-duplicate-box is-blocked"><h3>Unidades já cadastradas — importação bloqueada</h3>${databaseConflicts.map(({ incoming, existing }) => `<div><strong>${escapeHtml(incoming.unit)}</strong><span>Arquivo: venda ${escapeHtml(incoming.externalSaleId)} · ${escapeHtml(incoming.client)} · ${money(incoming.contractValue)}</span><span>Banco: ${escapeHtml(existing.client || "Cliente não informado")} · ${money(existing.contractValue || 0)} · ${escapeHtml(existing.status || "")}</span></div>`).join("")}</section>` : ""}${fileConflicts.length ? `<section class="sml-duplicate-box"><h3>Unidades repetidas no arquivo — escolha qual importar</h3>${fileConflicts.map((conflict) => `<fieldset><legend>${escapeHtml(conflict.unit)}</legend>${conflict.sales.map((sale) => `<label><input type="radio" name="sml-duplicate-${escapeHtml(conflict.unit)}" value="${sale.id}" data-sml-duplicate-choice="${escapeHtml(conflict.unit)}" ${state.smlFinanceDuplicateSelections[conflict.unit] === sale.id ? "checked" : ""}><span>Venda ${escapeHtml(sale.externalSaleId)} · ${escapeHtml(sale.client)} · ${money(sale.contractValue)} · ${escapeHtml(sale.realEstate)}</span></label>`).join("")}</fieldset>`).join("")}</section>` : ""}`;
  const previewModal = state.smlFinanceExtraction ? `<div class="modal-backdrop"><section class="modal-card sml-import-modal"><div class="panel-head"><div><h2>Prévia da importação SML</h2><p class="muted-copy">Confira os dados. Nenhum registro foi importado ainda.</p></div><button class="icon" type="button" data-close-sml-preview>×</button></div>${conflictBoxes}<div class="table-wrap"><table><thead><tr><th>Unidade</th><th>Cliente</th><th>Assinatura</th><th>Valor unidade</th><th>Sinal</th><th>Financiamento</th><th>Comissão Lev</th><th>Corretor</th><th>Validação</th></tr></thead><tbody>${preview.map((sale) => `<tr><td>${escapeHtml(sale.unit)}</td><td>${escapeHtml(sale.client)}</td><td>—</td><td>${money(sale.contractValue)}</td><td>${money(sale.signalValue || 0)}</td><td>${money(sale.financingValue || 0)}</td><td>${money(sale.commissionValue)}</td><td>${escapeHtml(sale.realEstate)}</td><td>${sale.duplicate ? '<span class="chip chip-warning">Já existe no banco</span>' : sale.fileConflict && !isSelectedFileSale(sale) ? '<span class="chip">Não selecionada</span>' : sale.fileConflict && unresolvedFileConflicts.some((item) => item.unit === sale.unit) ? '<span class="chip chip-warning">Escolha pendente</span>' : '<span class="status-ok">Pronta</span>'}</td></tr>`).join("")}</tbody></table></div><div class="metrics"><div class="metric"><span>Vendas válidas</span><strong>${importablePreview.length}</strong></div><div class="metric"><span>Valor de vendas</span><strong>${money(importablePreview.reduce((sum, sale) => sum + Number(sale.contractValue || 0), 0))}</strong></div><div class="metric"><span>Comissão calculada</span><strong>${money(importablePreview.reduce((sum, sale) => sum + Number(sale.commissionValue || 0), 0))}</strong></div></div><div class="row-actions"><button class="primary" type="button" data-confirm-sml-import ${unresolvedFileConflicts.length ? "disabled" : ""}>Importar vendas válidas</button><button type="button" data-close-sml-preview>Cancelar</button></div></section></div>` : "";
  renderShell(`
    ${renderViewHead("Financeiro SML", "Controle de recebíveis da Saint Michel", { actions: `<input id="smlFileInput" type="file" accept=".xlsx,.xls" hidden><button type="button" data-test-sml-authorization>Testar e-mail</button><button type="button" data-send-sml-authorization>Enviar para confirmação SML</button><button class="primary" type="button" data-open-sml-file>Importar XLSX</button>` })}
  ...
  document.querySelector("#smlFileInput")?.addEventListener("change", async (event) => { const file = event.target.files?.[0]; if (!file) return; try { const fileBase64 = await readFileAsDataUrl(file); state.smlFinanceDuplicateSelections = {}; state.smlFinanceExtraction = await api("/api/sml-finance/import-preview", { method: "POST", body: JSON.stringify({ fileBase64 }) }); renderSmlFinanceView(); } catch (error) { alert(error.message); } });
  document.querySelectorAll("[data-close-sml-preview]").forEach((button) => button.addEventListener("click", () => { state.smlFinanceExtraction = null; state.smlFinanceDuplicateSelections = {}; renderSmlFinanceView(); }));
  document.querySelectorAll("[data-sml-duplicate-choice]").forEach((input) => input.addEventListener("change", () => { state.smlFinanceDuplicateSelections[input.dataset.smlDuplicateChoice] = input.value; renderSmlFinanceView(); }));
  document.querySelector("[data-confirm-sml-import]")?.addEventListener("click", async (event) => { const salesToImport = importablePreview; if (unresolvedFileConflicts.length) return alert("Escolha qual venda importar para cada unidade repetida."); if (!salesToImport.length) return alert("Não há vendas novas para importar."); if (!confirm(`Importar ${salesToImport.length} venda(s) para o Financeiro SML?`)) return; try { setButtonBusy(event.currentTarget, true, "Importando..."); await api("/api/sml-finance/import", { method: "POST", body: JSON.stringify({ sales: salesToImport }) }); state.smlFinanceExtraction = null; state.smlFinanceDuplicateSelections = {}; await loadState(); state.view = "financeSml"; renderSmlFinanceView(); } catch (error) { setButtonBusy(event.currentTarget, false); alert(error.message); } });
  document.querySelector("[data-test-sml-authorization]")?.addEventListener("click", async (event) => {
    if (!confirm("Enviar um convite de teste para renat.cg@gmail.com? O teste não alterará o status das vendas.")) return;
    try {
      setButtonBusy(event.currentTarget, true, "Enviando teste...");
      const data = await api("/api/sml-finance/send-authorization-test", { method: "POST" });
      alert(data.sent ? `Teste enviado para ${data.to}. O acesso expira em ${dateLabel(data.expiresAt)}.` : `O convite de teste foi criado, mas o e-mail não foi enviado: ${data.reason}`);
  ...
  document.querySelector("[data-identify-sml-zero]")?.addEventListener("click", async (event) => {
    if (!confirm("Identificar como entrada zero somente as unidades cujo sinal seja inferior a R$ 1.000,00 e cuja soma de sinal + financiamento seja igual ao valor da unidade?")) return;
    try {
      setButtonBusy(event.currentTarget, true, "Verificando...");
      const data = await api("/api/sml-finance/identify-zero-entry", { method: "POST" });
      state.smlFinance = { ...state.smlFinance, sales: data.sales || [] };
      alert(`${data.identified || 0} unidade(s) identificada(s) com entrada zero.`);
      renderSmlFinanceView();
    } catch (error) {
      setButtonBusy(event.currentTarget, false);
      alert(error.message);
    }
  });
  document.querySelectorAll("[data-sml-zero-entry]").forEach((cell) => cell.addEventListener("contextmenu", (event) => {
    event.preventDefault();
    document.querySelectorAll(".sml-zero-context-menu").forEach((menu) => menu.remove());
    const menu = document.createElement("div");
    menu.className = "sml-zero-context-menu";
    menu.style.left = `${Math.min(event.clientX, window.innerWidth - 210)}px`;
    menu.style.top = `${Math.min(event.clientY, window.innerHeight - 60)}px`;
    menu.innerHTML = '<button type="button">Desmarcar entrada zero</button>';
    document.body.appendChild(menu);
    const closeMenu = (clickEvent) => { if (!menu.contains(clickEvent.target)) menu.remove(); };
    ...
    menu.querySelector("button")?.addEventListener("click", async () => {
      if (!confirm("Desmarcar esta unidade como entrada zero? Ela não será marcada novamente nas próximas verificações automáticas.")) return;
      try {
        const data = await api(`/api/sml-finance/sales/${encodeURIComponent(cell.dataset.smlZeroEntry)}/zero-entry`, { method: "PATCH" });
        state.smlFinance = { ...state.smlFinance, sales: (state.smlFinance.sales || []).map((sale) => sale.id === data.sale.id ? data.sale : sale) };
        menu.remove();
        renderSmlFinanceView();
      } catch (error) {
        alert(error.message);
...
function renderUnitManagementPage(project) {
  ...
  document.querySelectorAll("[data-inline-table-select-trigger]").forEach((button) => button.addEventListener("click", (event) => { event.stopPropagation(); const select = button.closest("[data-inline-table-select]"); document.querySelectorAll(".inline-table-select.open").forEach((item) => { if (item !== select) item.classList.remove("open"); }); const willOpen = !select?.classList.contains("open"); select?.classList.toggle("open", willOpen); const menu = select?.querySelector(".inline-table-select-menu"); if (willOpen && menu) { const rect = button.getBoundingClientRect(); menu.style.left = `${rect.left}px`; menu.style.top = `${rect.bottom + 4}px`; menu.style.width = `${Math.max(rect.width, 140)}px`; requestAnimationFrame(() => { const menuRect = menu.getBoundingClientRect(); if (menuRect.bottom > window.innerHeight - 8) menu.style.top = `${Math.max(8, rect.top - menuRect.height - 4)}px`; }); } }));
  ...
  document.querySelector(".unit-management-table")?.addEventListener("scroll", () => document.querySelectorAll(".inline-table-select.open").forEach((item) => item.classList.remove("open")));
...
function renderSmlExternalAuthorization() {
  ...
  const renderSales = () => {
    app.innerHTML = `<main class="sml-public-page"><section class="sml-public-content"><div class="sml-public-title"><h1>Vendas para confirmação</h1><p>Desmarque somente as vendas que não deseja autorizar. Após confirmar, a operação não poderá ser desfeita.</p></div><section class="metrics sml-public-totals"><div class="metric"><span>Vendas selecionadas</span><strong data-sml-selected-count>0</strong></div><div class="metric"><span>Valor de vendas</span><strong data-sml-selected-sales>${money(0)}</strong></div><div class="metric"><span>Valor da comissão</span><strong data-sml-selected-commission>${money(0)}</strong></div></section><label class="sml-public-select-all"><input type="checkbox" data-sml-public-select-all checked><span>Marcar/desmarcar todas</span></label><div class="table-wrap"><table class="access-table"><thead><tr><th>Confirmar</th><th>Unidade</th><th>Cliente</th><th>Assinatura</th><th>Valor unidade</th><th>Sinal</th><th>Financiamento</th><th>Comissão Lev</th><th>Corretor</th></tr></thead><tbody>${session.sales.map((sale) => `<tr><td><input class="sml-public-checkbox" type="checkbox" data-sml-public-sale="${sale.id}" checked></td><td>${escapeHtml(sale.unit)}</td><td>${escapeHtml(sale.client)}</td><td>—</td><td>${money(sale.contractValue)}</td><td>${money(sale.signalValue || 0)}</td><td>${money(sale.financingValue || 0)}</td><td>${money(sale.commissionValue)}</td><td>${escapeHtml(sale.realEstate || "")}</td></tr>`).join("") || '<tr><td colspan="9" class="empty">Não há vendas pendentes neste convite.</td></tr>'}</tbody></table></div><button class="primary sml-public-confirm" type="button" data-confirm-sml-public>Confirmar selecionadas</button></section></main>`;
    const selectedSales = () => session.sales.filter((sale) => document.querySelector(`[data-sml-public-sale="${sale.id}"]`)?.checked);
    const refresh = () => { const selected = selectedSales(); const checkboxes = [...document.querySelectorAll("[data-sml-public-sale]")]; const selectAll = document.querySelector("[data-sml-public-select-all]"); document.querySelector("[data-sml-selected-count]").textContent = selected.length; document.querySelector("[data-sml-selected-sales]").textContent = money(selected.reduce((sum, sale) => sum + Number(sale.contractValue || 0), 0)); document.querySelector("[data-sml-selected-commission]").textContent = money(selected.reduce((sum, sale) => sum + Number(sale.commissionValue || 0), 0)); if (selectAll) { selectAll.checked = Boolean(checkboxes.length) && checkboxes.every((checkbox) => checkbox.checked); selectAll.indeterminate = selected.length > 0 && selected.length < checkboxes.length; } };
    document.querySelectorAll("[data-sml-public-sale]").forEach((checkbox) => checkbox.addEventListener("change", refresh));
    document.querySelector("[data-sml-public-select-all]")?.addEventListener("change", (event) => { document.querySelectorAll("[data-sml-public-sale]").forEach((checkbox) => { checkbox.checked = event.currentTarget.checked; }); refresh(); });
    refresh();
    document.querySelector("[data-confirm-sml-public]")?.addEventListener("click", async (event) => { const selected = selectedSales(); if (!selected.length) return alert("Selecione pelo menos uma venda."); const salesTotal = selected.reduce((sum, sale) => sum + Number(sale.contractValue || 0), 0); const commissionTotal = selected.reduce((sum, sale) => sum + Number(sale.commissionValue || 0), 0); if (!confirm(`Tem certeza de que deseja confirmar ${selected.length} venda(s)?\n\nValor total das vendas: ${money(salesTotal)}\nValor total da comissão: ${money(commissionTotal)}\n\nDepois de confirmada, esta operação não poderá ser desfeita.`)) return; try { setButtonBusy(event.currentTarget, true, "Confirmando..."); const result = await api(`/api/sml-finance/public/${encodeURIComponent(token)}/confirm`, { method: "POST", body: JSON.stringify({ email: session.email, password: session.password, saleIds: selected.map((sale) => sale.id) }) }); app.innerHTML = result.test ? '<main class="sml-public-page"><section class="sml-public-success"><h1>Teste concluído</h1><p>O fluxo foi validado e nenhuma venda teve seu status alterado.</p></section></main>' : '<main class="sml-public-page"><section class="sml-public-success"><h1>Vendas confirmadas</h1><p>As vendas selecionadas foram registradas com sucesso.</p></section></main>'; } catch (error) { setButtonBusy(event.currentTarget, false); alert(error.message); } });
  };
  renderLogin();
}
